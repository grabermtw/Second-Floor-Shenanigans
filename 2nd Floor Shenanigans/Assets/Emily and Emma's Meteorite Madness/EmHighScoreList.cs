...
public class EmHighScoreList : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < 10; i++)
        {
            highScores.Add(new HighScoreEntry(PlayerPrefs.GetInt("EmInt" + i, 0),
                PlayerPrefs.GetString("EmString" + i)));
        }
    ...
    public void Save()
    {
        for (int i = 0; i < 10; i++)
        {
            PlayerPrefs.SetInt("EmInt" + i, ((HighScoreEntry)highScores[i]).GetScore());
            PlayerPrefs.SetString("EmString" + i, ((HighScoreEntry)highScores[i]).GetName());
        }
    }
}
