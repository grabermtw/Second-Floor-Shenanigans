...
public class OmerHighScores : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < 10; i++)
        {
            highScores.Add(new HighScoreEntry(PlayerPrefs.GetInt("OmerInt" + i, 0),
                PlayerPrefs.GetString("OmerString" + i)));
        }
    ...
    public void Save()
    {
        for (int i = 0; i < 10; i++)
        {
            PlayerPrefs.SetInt("OmerInt" + i, ((HighScoreEntry)highScores[i]).GetScore());
            PlayerPrefs.SetString("OmerString" + i, ((HighScoreEntry)highScores[i]).GetName());
        }
    }
