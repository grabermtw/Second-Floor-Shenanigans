...
public class NateHighScoreList : MonoBehaviour {
    ...
    void Start()
    {
        for (int i = 0; i < 10; i++)
        {
            highScores.Add(new HighScoreEntry(PlayerPrefs.GetInt("NateInt" + i, 0),
                PlayerPrefs.GetString("NateString" + i)));
        }
    ...
    public void Save()
    {
        for (int i = 0; i < 10; i++)
        {
            PlayerPrefs.SetInt("NateInt" + i, ((HighScoreEntry)highScores[i]).GetScore());
            PlayerPrefs.SetString("NateString" + i, ((HighScoreEntry)highScores[i]).GetName());
        }
    }
}
