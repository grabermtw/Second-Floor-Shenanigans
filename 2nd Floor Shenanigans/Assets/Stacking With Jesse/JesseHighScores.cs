...
public class JesseHighScores : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < 10; i++)
        {
            highScores.Add(new HighScoreEntry(PlayerPrefs.GetInt("JesseInt" + i, 0),
                PlayerPrefs.GetString("JesseString" + i)));
        }
    ...
    public void Save()
    {
        for (int i = 0; i < 10; i++)
        {
            PlayerPrefs.SetInt("JesseInt" + i, ((HighScoreEntry)highScores[i]).GetScore());
            PlayerPrefs.SetString("JesseString" + i, ((HighScoreEntry)highScores[i]).GetName());
        }
    }
}
